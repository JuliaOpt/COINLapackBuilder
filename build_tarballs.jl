...
mkdir build
cd build/
../configure --prefix=$prefix --with-pic --disable-pkg-config --with-blas="$prefix/lib/libcoinblas.a -lgfortran" --host=${target} --enable-shared --enable-static --enable-dependency-linking lt_cv_deplibs_check_method=pass_all
make -j${nproc}
make install
